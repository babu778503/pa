...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const switchView = async (view) => {
        ...
        if (view === 'your-work') {
            try {
                ...
                console.error("Failed to load 'My Work' view:", error);
                yourWorkView.innerHTML = `<p style="text-align: center; padding: 2rem;">Could not load this feature. Please try again later.</p>`;
            }
        }
    ...
    const triggerAlarm = (alarmId) => {
        ...
        if (Notification.permission === 'granted') { new Notification('ToolHub Reminder', { body: `Your reminder for "${alarmData.toolName}" is now!`, icon: 'logo.png' }); }
    ...
    const setAlarmWithDate = (toolId, toolName, scheduledDate, frequency) => {
        ...
        if (currentView === 'your-tools') renderYourToolsView(); if (currentView === 'your-work') { calendarDisplayDate = new Date(scheduledTime); switchView('your-work'); }
    };
    ...
    const handleDataViewClick = (e) => { const link = e.target.closest('a[data-view]'); if (link && link.id !== 'profile-link') { e.preventDefault(); const view = link.dataset.view; history.pushState({view}, '', view === 'home' ? '/' : `/${view}`); switchView(view); } };
    ...
    const handleRouteChange = () => {
        ...
        if (toolMatch) {
            ...
            hideTool(false);
            const view = path.substring(1) || 'home';
            switchView(view);
        }
    };
    ...
    window.addEventListener('popstate', (e) => {
        ...
        if (toolMatch) {
            const toolId = toolMatch[1];
            const tool = toolsData.find(t => t.id === toolId);
            if (tool) showTool(tool.id, tool.Name, false);
        } else {
            hideTool(false);
            const view = e.state?.view || path.substring(1) || 'home';
            switchView(view);
        }
    });
    ...
    const handleSearch = () => {
        ...
        if (query.length === 0) {
            ...
            if (currentView === 'home' || currentView === 'popular') popularToolsSection.style.display = '';
            if (currentView === 'home') newToolsSection.style.display = '';
            return;
        }
        ...
        searchResultsHeading.innerHTML = `Found ${filteredTools.length} tools for "<strong>${sanitizeHTML(query)}</strong>"`;
        const emptyMessage = `Found 0 tools for "<strong>${sanitizeHTML(query)}</strong>". Please email the tool name to <a href="mailto:aktar.babu@gmail.com">aktar.babu@gmail.com</a>. Your requested tool will be added shortly with Name.<div class="idea-credit"><i class="fas fa-lightbulb"></i> Idea Share: Ayra, New York, USA</div>`;
        renderTools(searchResultsGrid, filteredTools, emptyMessage);
    };
    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (yourWorkView.style.display !== 'none') {
            const deleteEventBtn = e.target.closest('.delete-event-btn'); 
            const calendarEvent = e.target.closest('.calendar-event');
            if (e.target.closest('#calendar-today-btn')) { calendarDisplayDate = new Date(); switchView('your-work'); }
            if (e.target.closest('#calendar-prev-btn')) { calendarDisplayDate.setDate(calendarDisplayDate.getDate() - 1); switchView('your-work'); }
            if (e.target.closest('#calendar-next-btn')) { calendarDisplayDate.setDate(calendarDisplayDate.getDate() + 1); switchView('your-work'); }
            if (deleteEventBtn) {
                e.stopPropagation(); 
                ...
                if (alarmId && confirm(message)) { delete activeAlarms[alarmId]; saveAlarms(); updateYourWorkBadge(); switchView('your-work'); }
